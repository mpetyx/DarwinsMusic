...
class _Network(object):
    ...
    def __init__(self, name, homepage, ws_server, api_key, api_secret, session_key, submission_server, username,
                 password_hash,
                 domain_names, urls):
        """
    ...
    def get_scrobbler(self, client_id, client_version):
        ...
        _deprecation_warning(
            "Use _Network.scrobble(...), _Network.scrobble_many(...), and Netowrk.update_now_playing(...) instead")
    ...
    def scrobble(self, artist, title, timestamp, album=None, album_artist=None, track_number=None,
        ...
        return self.scrobble_many(
            ({"artist": artist, "title": title, "timestamp": timestamp, "album": album, "album_artist": album_artist,
              "track_number": track_number, "duration": duration, "stream_id": stream_id, "context": context,
              "mbid": mbid},))
...
class _BaseObject(object):
    ...
    def __hash__(self):
        return hash(self.network) + \
               hash(str(type(self)) + "".join(
                   list(self._get_params().keys()) + list(self._get_params().values())).lower())
...
class Playlist(_BaseObject):
    ...
    def get_url(self, domain_name=DOMAIN_ENGLISH):
        ...
        * domain_name: The network's language domain. Possible values:
        ...
        return self.network._get_url(domain_name, "playlist") % {'appendix': appendix,
                                                                 "user": self.get_user().get_name()}
...
class Track(_BaseObject, _Taggable):
    ...
    def get_url(self, domain_name=DOMAIN_ENGLISH):
        ...
        * domain_name: The network's language domain. Possible values:
        ...
        return self.network._get_url(domain_name, "track") % {'domain': self.network._get_language_domain(domain_name),
