...
for f in files:
    ...
    if track['mbid'] == '':
        ...
        url = "http://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=9d8eec456625c74e02a7e23bd1d7c68a&artist=" + quote(
            track['artist']['name'].encode('utf-8')) + "&track=" + quote(track["name"].encode('utf-8')) + "&format=json"
    else:
        mbid = track['mbid']
    ...
    json.dump(trackInfo, file("../data/" + mbid + ".json", 'w'))
    sleep(0.5)
